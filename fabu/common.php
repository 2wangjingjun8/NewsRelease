 ...
 *                                      }
 */
function files_upload($name="picture",$save_dir="images",$allow_suffix=array('jpg','jpeg','gif','png')){
    //如果是单文件上传，改变数组结构
    ...
    for($i=0;$i <$num;$i++)
    {
        ...
        $response['imgName'][] =$new_file_name;
...
function getPictureUrl($files, $field = 'picture'){
	$picture = "../../images/ma05.jpg";
	// 上传图片
	if ($files) {
		$uploadRes = files_upload($field,'../../news/images/');
		if ($uploadRes['status']) {
			// echo "上传成功";
			$picture = '../../news/images/'.$uploadRes['imgName'][0];
		}
    }
    return $picture;
}
